...
func parseWithLocation(name string, timeStr string) (time.Time, error) {
	...
	if l, err := time.LoadLocation(locationName); err != nil {
		...
		fmt.Println(locationName, lt.Format(TIME_LAYOUT))
		return lt, nil
	}
}
func testTime() {
	location, _ := time.LoadLocation("Asia/Shanghai")
	location2, _ := time.LoadLocation("America/Cordoba")
	now := time.Now()
	fmt.Println("0. now: ", now.In(location).Format(TIME_LAYOUT))
	fmt.Println("0. now2: ", now.In(location2).Format(TIME_LAYOUT))
